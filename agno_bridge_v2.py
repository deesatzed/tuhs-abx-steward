...
class TUHSGuidelineLoader:
    ...
    def build_agent_instructions(self, infection_name: str, subsection_filter: str = None) -> List[str]:
        ...
        instructions.append("")
        instructions.append("📋 ALLERGY CLASSIFICATION:")
        instructions.append("- MILD PCN allergy: 'Rash', 'rash only', 'mild rash', 'itching', 'mild hives' (without anaphylaxis)")
        instructions.append("- SEVERE PCN allergy: 'Anaphylaxis', 'SJS', 'DRESS', 'angioedema', 'severe hives', 'shortness of breath'")
        instructions.append("")
        instructions.append("🔵 FOR MILD PCN ALLERGY (rash only):")
        instructions.append("- CAN use cephalosporins (Ceftriaxone, Cefepime, Cefazolin) - cross-reactivity <1%")
        instructions.append("- CAN use piperacillin-tazobactam if no better alternative")
        instructions.append("- Follow the 'NO Penicillin ALLERGY' or 'Mild-Moderate PCN Allergy' guideline regimens")
        instructions.append("- Example: For intra-abdominal with mild rash → Piperacillin-tazobactam + Metronidazole is appropriate")
        instructions.append("")
        instructions.append("🔴 FOR SEVERE PCN ALLERGY (anaphylaxis/SJS/DRESS):")
        instructions.append("- MUST avoid ALL penicillins AND all cephalosporins")
        instructions.append("- Cefepime, Ceftriaxone, Cefazolin = ALL CEPHALOSPORINS = CONTRAINDICATED")
        instructions.append("- Use ONLY: Aztreonam, Fluoroquinolones, Vancomycin, Aminoglycosides")
        instructions.append("- Follow the 'Severe PCN Allergy' guideline regimens")
        instructions.append("- Example: For intra-abdominal with anaphylaxis → Aztreonam + Metronidazole + Vancomycin")
        instructions.append("")
        instructions.append("⚠️  CRITICAL RULES:")
        instructions.append("- Do NOT say 'Drug X is contraindicated' - just select the appropriate regimen based on allergy severity")
        instructions.append("- NEVER include contraindicated drugs in your recommendation, not even as alternatives")
        instructions.append("- If allergy description is unclear, assume SEVERE and err on the side of safety")
